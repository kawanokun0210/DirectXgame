...
void Object::Draw(const Vector4& material, Transform& transform, uint32_t index, Camera* cameraTransform, const DirectionalLight& light, bool isLighting)
{
	...
	transform.matWorld = MakeIdentity4x4();
	...
	transform.matWorld = worldMatrix;

	if (parent) {
		transform.matWorld = Multiply(transform.matWorld, parent->matWorld);
	}
	...
	*wvpData_ = { wvpMatrix_,worldMatrix,scaleMatrix,transform.matWorld };
