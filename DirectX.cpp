...
IDXGIFactory7* DirectX::dxgiFactory;

HRESULT DirectX::hr;

//使用するアダプタ用の変数
IDXGIAdapter4* DirectX::useAdapter;

//D3D12Deviceの生成
ID3D12Device* DirectX::device;

//コマンドキュー生成
ID3D12CommandQueue* DirectX::commandQueue;
//コマンドアロケータの生成
ID3D12CommandAllocator* DirectX::commandAllocator;
//コマンドリストを生成する
ID3D12GraphicsCommandList* DirectX::commandList;

//スワップチェーン
IDXGISwapChain4* DirectX::swapChain;

//SwapChainからResourceを引っ張ってくる
ID3D12Resource* DirectX::swapChainResources[2];

//ディスクリプタヒープの生成
ID3D12DescriptorHeap* DirectX::rtvDescriptorHeap;

//Fence
ID3D12Fence* DirectX::fence;
uint64_t DirectX::fenceValue;
HANDLE DirectX::fenceEvent;

//debug
ID3D12Debug1* DirectX::debugController;
IDXGIDebug1* DirectX::debug;

void DirectX::Log(const std::string& message)
{
